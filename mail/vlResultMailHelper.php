...
if(isset($_POST['toEmail']) && trim($_POST['toEmail'])!=''){
   if(isset($mailconf['rs_field']) && trim($mailconf['rs_field'])!= ''){
      ...
      if($mail->send()){
           ...
           $_SESSION['alertMsg']='Successfully Sent Email.';
           error_log("Mailer Error: " . $mail->ErrorInfo);
           header('location:vlResultMail.php');
      }
   }else{
      $_SESSION['alertMsg']='Unable to send mail. Please try later. 2';
      header('location:vlResultMail.php');
   }
}else{
   $_SESSION['alertMsg']='Unable to send mail. Please try later. 3';
  header('location:vlResultMail.php');
}