...
if(isset($_POST['toEmail']) && trim($_POST['toEmail'])!= '' && count($_POST['sample']) >0){
     ...
     if(isset($mailconf['rq_field']) && trim($mailconf['rq_field'])!= ''){
         ...
         $allField = array('Specimen ID','Urgency','Region','District','Clinic Name','Clinician Name','Specimen Collection Date','Specimen Received Date','Collected by (Initials)','Gender','Date Of Birth','Age in years','Age in months','Is Patient Pregnant?','Is Patient Breastfeeding?','Patient OI/ART Number','Date Of ART Initiation','ART Regimen','Patient consent to SMS Notification?','Patient Mobile Number','Date Of Last Viral Load Test','Result Of Last Viral Load','Viral Load Log','Reason For VL Test','Lab Name','LAB No','VL Testing Platform','Specimen type','Specimen Testing Date','Viral Load Result(copiesl/ml)','Log Value','If no result','Rejection Reason','Reviewed By','Verified By','Laboratory Scientist Comments','Status');
         ...
         for($s=0;$s<count($_POST['sample']);$s++){
            $row = array();
            $sampleQuery="SELECT serial_no FROM vl_request_form as vl LEFT JOIN facility_details as f ON vl.facility_id=f.facility_id where vl.vl_sample_id = '".$_POST['sample'][$s]."' ORDER BY f.facility_name ASC";
...
<div class="content-wrapper">
    <div class="box box-default">
        <div class="box-header with-border">
          <div style="text-align:center;"><h4>Site Name : <?php echo ucwords($_POST['toName']); ?></h4></div>
        </div>
        <div class="box-body">
               ...
               <div class="row">
                  <div class="col-lg-12" style="text-align:center !important;">
                     <table class="table table-bordered table-striped" style="width:18%;margin-left:41%;">
                         <thead>
                           <tr>
                             <th style="text-align:center;background-color:#71b9e2;color:#FFFFFF;">Selected Specimen</th>
                           </tr>
                         </thead>
                         <tbody>
                           <?php
                           ...
                           ?>
                            <tr>
                             <td style="text-align:left;"><?php echo $sampleResult[0]['serial_no']; ?></td>
                            </tr>
                           <?php } ?>
                         </tbody>
                     </table>
