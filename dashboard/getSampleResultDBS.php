...
$sWhere.= ' where DATE(vl.sample_collection_date) <= "'.$cDate.'" AND DATE(vl.sample_collection_date) >= "'.$lastSevenDay.'" AND vl.vlsm_country_id = "'.$configFormResult[0]['value'].'" AND vl.sample_type = "7"';
...
//get waiting data
$waitingQuery="select COUNT(vl_sample_id) as total FROM vl_request_form as vl where vl.vlsm_country_id = '".$configFormResult[0]['value']."' " . " AND (vl.result is null or vl.result = '') AND (vl.sample_type = '7')";
...
foreach($vlResult as $vlData){
   $tQuery="select COUNT(vl_sample_id) as total FROM vl_request_form as vl where vl.vlsm_country_id = '".$configFormResult[0]['value']."' AND vl.sample_type = '7'";
...
$stWhere.= ' where DATE(vl.sample_tested_datetime) <= "'.$cDate.'" AND DATE(vl.sample_tested_datetime) >= "'.$lastSevenDay.'" AND vl.vlsm_country_id = "'.$configFormResult[0]['value'].'"  AND vl.sample_type = "7" ';
