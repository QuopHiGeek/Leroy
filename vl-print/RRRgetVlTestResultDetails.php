        ...
        if (isset($_POST['iSortCol_0'])) {
            ...
            for ($i = 0; $i < intval($_POST['iSortingCols']); $i++) {
                if ($_POST['bSortable_' . intval($_POST['iSortCol_' . $i])] == "true") {
                    $sOrder .= $orderColumns[intval($_POST['iSortCol_' . $i])] . " " . ( $_POST['sSortDir_' . $i] ) . ", ";
                }
            }
	...
	$t_end_date = '';
		...
		if (isset($sWhere) && $sWhere != "") {
			...
			if(isset($_POST['specimenType']) && trim($_POST['specimenType'])!= ''){
                ...
                if ( $_SESSION['goBackPage'] == 'vlPrintResult.php' ) {
                    $sWhere = $sWhere.' AND s.sample_id in (SELECT sample_id FROM r_sample_type where status="active" and patient_group = 0)';
                }
                if ( $_SESSION['goBackPage'] == 'vlPrintEIDResult.php' ) {
                    $sWhere = $sWhere.' AND s.sample_id in (SELECT sample_id FROM r_sample_type where status="active" and patient_group = 1)';
                }
            }
			...
			if(isset($_POST['specimenType']) && trim($_POST['specimenType'])!= ''){
			...
			}else if ($_SESSION['goBackPage'] != 'vlTestResult.php') {
				if(isset($setWhr)){ 
                    $sWhere = $sWhere.'AND ';
                }
				if(!isset($setWhr)){
                ...
                if ( $_SESSION['goBackPage'] == 'vlPrintResult.php' ) {
                    $sWhere = $sWhere.' s.sample_id in (SELECT sample_id FROM r_sample_type where status="active" and patient_group = 0)';
                }
                if ( $_SESSION['goBackPage'] == 'vlPrintEIDResult.php' ) {
                    $sWhere = $sWhere.' s.sample_id in (SELECT sample_id FROM r_sample_type where status="active" and patient_group = 1)';
                }
            }
			if(isset($_POST['siteName']) && trim($_POST['siteName'])!= ''){
			    if(isset($setWhr)){
	    ...
	    // echo($_SESSION['goBackPage'].'~'.$sQuery); die();
	   ...
	   $row[] = ucwords($aRow['patient_location']);
        ...
        $toPrint = '';
	    if(isset($_POST['vlPrint']) && $_POST['vlPrint']=='print'){
        $toPrint = '<a href="javascript:void(0);" class="btn btn-primary btn-xs" style="margin: 2px;" title="View" onclick="convertResultToPdf('.$aRow['vl_sample_id'].');"><i class="fa fa-print"> Print</i></a>';
      }else{
      ...
      $toEnterResult = '<a href="updateVlTestResult.php?id=' . base64_encode($aRow['vl_sample_id']) . '" class="btn btn-success btn-xs" style="margin-right: 2px;" title="Result"><i class="fa fa-pencil-square-o"></i> Enter Result</a>';
      $row[] = $toEnterResult.$toPrint;
