...
$primaryKey="vl_sample_id";
        ...
        */
        ...
        $aColumns = array('vl.serial_no','b.batch_code','vl.patient_art_no','vl.patient_first_name','vl.patient_last_name','f.facility_name','s.sample_name','vl.result',"DATE_FORMAT(vl.last_modified_datetime,'%d-%b-%Y')",'ts.status_name', 'vl.current_regimen');
        $orderColumns = array('vl.serial_no','b.batch_code','vl.patient_art_no','vl.patient_first_name','vl.patient_last_name','f.facility_name','s.sample_name','vl.result','vl.last_modified_datetime','ts.status_name', 'vl.current_regimen');
	...
	$t_end_date = '';
		...
		if (isset($sWhere) && $sWhere != "") {
			...
			if(isset($_POST['siteName']) && is_array($_POST['siteName'])){
                //echo($_POST['siteName']); die;
                $sitesNames = '';
                foreach($_POST['siteName'] as $key=>$val) {
                    $sitesNames = $sitesNames.$val;
                    if($key < $_POST['siteName'].lenght) {
                        $sitesNames = $sitesNames.',';
                    }
                }
			    if(isset($setWhr)){
			      $sWhere = $sWhere.' AND f.facility_id IN ('.$sitesNames.')';
			    }else{
			      $setWhr = 'where';
			      $sWhere=' where '.$sWhere;
			      $sWhere = $sWhere.' f.facility_id IN ('.$sitesNames.')';
			    }
			}
			if(isset($_POST['artNo']) && trim($_POST['artNo'])!= ''){
				if(isset($setWhr)){
        ...
        // $sQuery = $sQuery." "."AND vl.result NOT in ('FAILED') and vl.result is not null ";
        if (isset($_POST['collectionDate']) && $_POST['collectionDate']!='') {
            //echo ($_POST['collectionDate']); die;
            $str_arr = explode (" to ", $_POST['collectionDate']);
            //echo ($str_arr[0]." ".$str_arr[1]); die;
            $sQuery = $sQuery." "."AND vl.sample_tested_datetime between DATE('".$str_arr[0]."') and DATE('".$str_arr[1]."')";
            //echo ($sQuery); die;
        }
        if (isset($_POST['pivotDS']) && $_POST['pivotDS'] == 'Virally Suppressed') {
            //echo ($_POST['pivotDS']); die;
            $sQuery = $sQuery." "."AND (vl.result in ('Below Detection Level', 'Target Not Detected', '<20')";
            $sQuery = $sQuery." "."OR vl.result <= 1000 )";
            //echo ($sQuery); die;
        }

        if (isset($_POST['pivotDS']) && $_POST['pivotDS'] == 'Virally NOT Suppressed') {
            //echo ($_POST['pivotDS']); die;
            $sQuery = $sQuery." "."AND vl.result > 1000 ";
            //echo ($sQuery); die;
        }

        if (isset($_POST['pivotDS']) && $_POST['pivotDS'] == 'Total number of viral load tests done') {
            //echo ($_POST['pivotDS']); die;
            $sQuery = $sQuery." "."AND (vl.result not in ('Below Detection Level', 'Target Not Detected', '<20')";
            $sQuery = $sQuery." "."AND vl.result >= 1000 )";
            //echo ($sQuery); die;
        }

        // if (isset($_POST['region']) && $_POST['region']!='') {
        //     //echo ($_POST['region']); die;
        //     $sQuery = $sQuery." "."AND (f.facility_state in (".$_POST['region'].")";
        //     $sQuery = $sQuery." "."AND vl.result >= 1000 )";
        // }
        if(isset($_POST['region']) && is_array($_POST['region'])){
                ...
                foreach($_POST['region'] as $key=>$val) {
                    $regionNames = $regionNames."'$val'";
                    if($key < $_POST['region'].lenght) {
                        $regionNames = $regionNames.',';
                    }
                }
                $sQuery = $sQuery.' AND f.facility_state IN ('.$regionNames.')';
			    
            //echo ($sQuery); die;
                
        }

        // $sQuery = $sQuery." AND vl.patient_age_in_years not in ('ADULT')";
        // $sQuery = $sQuery." AND vl.patient_age_in_years is not NULL";

        if(isset($_POST['age']) && trim($_POST['age'])!= ''){
                $sQuery = $sQuery." AND vl.patient_age_in_years BETWEEN ".$_POST['age']."";
        }
        
        $sQuery = $sQuery.' AND ( vl.result_status = 7 )';
        ...
        */
        $output = array(
            "sEcho" => intval(isset($_POST['sEcho']) ? $_POST['sEcho'] : ''),
	    ...
	    //    $aRow['last_modified_datetime'] = $general->humanDateFormat($xplodDate[0])." ".$xplodDate[1];
	       $last_modified_datetime = $general->humanDateFormat($xplodDate[0])." ".$xplodDate[1];
	    }else{
	    ...
	    $last_modified_datetime = $aRow['last_modified_datetime'];
        $status_name = ucwords($aRow['status_name']);
	    $current_regimen = $aRow['current_regimen'];
	    ...
	    //   $row[] = '<a href="javascript:void(0);" class="btn btn-primary btn-xs" style="margin-right: 2px;" title="View" onclick="convertResultToPdf('.$aRow['vl_sample_id'].');"><i class="fa fa-print"> Print</i></a>';
	       $rowLink = '<a href="javascript:void(0);" class="btn btn-primary btn-xs" style="margin-right: 2px;" title="View" onclick="convertResultToPdf('.$aRow['vl_sample_id'].');"><i class="fa fa-print"> Print</i></a>';
	    }else{
	    ...
	    //  $row[] = '<a href="updateVlTestResult.php?id=' . base64_encode($aRow['vl_sample_id']) . '" class="btn btn-success btn-xs" style="margin-right: 2px;" title="Result"><i class="fa fa-pencil-square-o"></i> Enter Result</a>';
	      $rowLink = '<a href="updateVlTestResult.php?id=' . base64_encode($aRow['vl_sample_id']) . '" class="btn btn-success btn-xs" style="margin-right: 2px;" title="Result"><i class="fa fa-pencil-square-o"></i> Enter Result</a>';
	    }
           $row = array(
         //   "serial_no" => $aRow['serial_no'],
          //  "batch_code" => $aRow['batch_code'],
          // "ART Number" => $aRow['patient_art_no'],
           //  "rowName" => ucwords($aRow['patient_first_name']).' '.ucwords($aRow['patient_middle_name']).' '.ucwords($aRow['patient_last_name']),
           "Gender" => $aRow['patient_gender'],
           "Age In Years" => $aRow['patient_age_in_years'],
           "Facility Name" => ucwords($aRow['facility_name']),
           "Specimen" => ucwords($aRow['sample_name']),
           "ANC Status" => $aRow['is_patient_pregnant'],
            "Results" => $aRow['fmtResult'],
