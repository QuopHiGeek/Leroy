...
$sWhere = '';
   ...
   $hvlQuery = $vlSampleQuery.' '.$sWhere ." AND vl.result = 'HIV-1/Detected' AND vl.result!=''";
   $lvlQuery = $vlSampleQuery.' '.$sWhere ." AND vl.result = 'Not Detected' AND vl.result!='' AND vl.result_status != '4'";
   //echo($hvlQuery); die();
   ...
   $tatSampleQuery="select DATE_FORMAT(DATE(sample_collection_date), '%b-%Y') as monthDate,CAST(ABS(AVG(TIMESTAMPDIFF(DAY,sample_tested_datetime,sample_collection_date))) AS DECIMAL (10,2)) as AvgDiff, DATE(vl.sample_collection_date) sampleDate from vl_request_form as vl INNER JOIN r_sample_status as ts ON ts.status_id=vl.result_status LEFT JOIN facility_details as f ON vl.facility_id=f.facility_id LEFT JOIN r_sample_type as s ON s.sample_id=vl.sample_type LEFT JOIN batch_details as b ON b.batch_id=vl.sample_batch_id where (vl.sample_collection_date is not null AND vl.sample_collection_date != '' AND DATE(vl.sample_collection_date) !='1970-01-01' AND DATE(vl.sample_collection_date) !='0000-00-00')
                        ...
                        AND DATE(vl.sample_collection_date) <= '".$end_date."' AND vl.vlsm_country_id='".$configFormResult[0]['value']."' group by monthDate, sampleDate order by sampleDate";
   $sWhere = '';
   if(isset($_POST['batchCode']) && trim($_POST['batchCode'])!= ''){
      $sWhere.= ' AND b.batch_code = "'.$_POST['batchCode'].'"';
   }
     ...
     colors: ['#FF0000', '#50B432']
      ...
      $('#samplesVlOverview').highcharts({
        ...
        series: [{
            colorByPoint: true,
            data: [
         {name:'HIV-1/Detected',y:<?php echo ucwords($vlSampleResult['hvl'][0]['total']);?>},
         {name:'Not Detected',y:<?php echo ucwords($vlSampleResult['lvl'][0]['total']);?>},
            ]
        }]
      });
