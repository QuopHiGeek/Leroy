...
$general=new Deforay_Commons_General();
	...
	$sQuery="select b.request_created_datetime ,b.batch_code, b.batch_id,b.post_notifies,count(vl.sample_code) as sample_code from vl_request_form vl right join batch_details b on vl.sample_batch_id = b.batch_id";
	...
	$sQuery = $sQuery.' '.$sWhere;
       ...
       // print_r($rResult);
        ...
        $aResultFilterTotal =$db->rawQuery("select b.siteID, b.request_created_datetime, b.batch_code, b.batch_id,b.post_notifies,count(vl.sample_code) as sample_code from vl_request_form vl right join batch_details b on vl.sample_batch_id = b.batch_id  $sWhere group by b.batch_id order by $sOrder");
        $iFilteredTotal = count($aResultFilterTotal);

        /* Total data set length */
        $aResultTotal =  $db->rawQuery("select b.siteID, b.request_created_datetime, b.batch_code, b.batch_id,b.post_notifies,count(vl.sample_code) as sample_code from vl_request_form vl right join batch_details b on vl.sample_batch_id = b.batch_id where vl.vlsm_country_id ='".$configResult[0]['value']."' group by b.batch_id");
	...
	if(isset($_SESSION['privileges']) && (in_array("editBatch.php", $_SESSION['privileges']))){
	   ...
	   //  if($aRow['post_notifies'] == 0){
	     //   $sit = ($_SESSION['userName'] == "Admin") ? $aRow['siteID'] : $_SESSION['siteID'];
           // $notifyGH='<a href="javascript:void(0);" id="sese" class="btn btn-info btn-xs" style="margin-right: 2px;" title="Notify GH Post" onclick="notifyGHPost(\''.$sit.','.$aRow['batch_id'].'\');"><i class="fa fa-qrcode"> Notify GHPost</i></a>';
       // }else{
        //    $notifyGH='<a href="javascript:void(0);" class="btn btn-default btn-xs" style="margin-right: 2px;" title="Notify GH Post" ;"><i class="fa fa-qrcode"> GHPost Notified</i></a>';
      //  }
	    $date = '';
	...
	//	    </select>';
		...
		if($batch){
		    $row[] = '<a href="editBatch.php?id=' . base64_encode($aRow['batch_id']) . '" class="btn btn-primary btn-xs" style="margin-right: 2px;" title="Edit"><i class="fa fa-pencil"> Edit</i></a>&nbsp;'.$printBarcode.'&nbsp;'.$notifyGH.'&nbsp;'.$editPosition;
		}
	    }
            $output['aaData'][] = $row;
        }
